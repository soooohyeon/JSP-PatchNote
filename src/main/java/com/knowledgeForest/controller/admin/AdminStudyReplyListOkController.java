...
public class AdminStudyReplyListOkController implements Execute {
...
//		검색 결과 데이터를 담을 리스트 선언
		List<StudyCommentDTO> studyReplyList = null;
//		검색어 파라미터로 받기
...
//		키워드 여부로 유저 목록 조회
		...
		studyReplyList = adminDAO.selectStudyReplyAll(paramMap);

//		총 공지 글 수
		total = adminDAO.getStudyReplyTotal(keyword);
...
//	    목록 설정
		request.setAttribute("studyReplyList", studyReplyList);
		result.setPath("/html/admin/admin-studycommentlist.jsp");
