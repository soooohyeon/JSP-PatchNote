...
public class StudyCommentWriteOkController implements Execute {
	...
	public Result execute(HttpServletRequest request, HttpServletResponse response)
			...
			//필수 파라미터 확인
			if(!jsonObject.has("studyNum") || !jsonObject.has("replyComment")) {
			...
			//DTO 설정
			studyCommentDTO.setStudyNum(jsonObject.get("studyNum").getAsInt());
			studyCommentDTO.setUserNum(userNum);
			studyCommentDTO.setStudyComment(jsonObject.get("replyComment").getAsString());
			
			System.out.println("studyCommentDTO 확인 : " + studyCommentDTO);
			
			//DB 저장
			studyCommentDAO.insert(studyCommentDTO);
