...
// 삭제 버튼 클릭시
function clickDeleteBtn(studyCommentNum) {
	// 확인, 취소 버튼 선택
	const isdelete = confirm('정말 삭제하시겠습니까?');
	console.log("studyCommentNum : " + studyCommentNum);
	// 확인 버튼 클릭시
	if (isdelete) {
		$.ajax({
			/* 스터디 댓글 삭제 컨트롤러로 이동, 댓글 넘버 쿼리스트링으로 전달 */
			url: getContextPath() + "/admin/admin-studycommentDeleteOk.ad?studyCommentNum=" + studyCommentNum,
			type: "GET",
			/* 스터디 댓글 삭제 성공 시 알람창 뜨면서 목록 페이지로 이동 */
			success: () => {
				alert('삭제가 완료되었습니다.');
				location.href = getContextPath() + "/admin/admin-studycommentlist.ad";
			},
			/* 삭제 불가시 알람창 */
